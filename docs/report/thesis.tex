...
\hline
M1: Project Plan & Week 2 & Week 2 & ✓ & BUILD\_PLAN.md, environment \\
M2: Foundation & Week 4 & Week 4 & ✓ & Utils, base classes, tests \\
M3: Core Monitors & Week 7 & Week 7 & ✓ & CPU, memory, disk modules \\
M4: All Monitors & Week 10 & Week 10 & ✓ & Network, GPU, sensors, process \\
M5: Functional TUI & Week 12 & Week 12 & ✓ & Integrated application \\
M6: Tests Passing & Week 13 & Week 13 & ✓ & 142 tests passing \\
M7: Quality Met & Week 14 & Week 14 & ✓ & 74\% coverage achieved \\
M8: Project Done & Week 16 & Week 16 & ✓ & Complete deliverables \\
\hline
\end{tabular}
\end{table}
...
Figure~\ref{fig:modules} details the relationships between specific modules within systop's architecture.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{images/module_interaction.png}
\caption{Detailed module interaction showing inheritance hierarchies, dependencies, and communication patterns}
\label{fig:modules}
\end{figure}
...
This unidirectional data flow (monitor → widget) simplifies reasoning about state and testing—monitors never know about widgets, enabling independent testing.
...
\begin{itemize}
    \item Step 6: CPU monitor + CPU widget + minimal app → Verified single monitor works
    \item Step 7-8: Added memory and disk → Verified three monitors coexist
    \item Step 9-12: Added remaining monitors → Full feature set verified
\end{itemize}
...
\hline
\texttt{test\_cpu\_collect} & Call \texttt{CPUMonitor.collect()} & Dict with \texttt{overall\_percent}, \texttt{per\_core\_percent}, valid ranges & ✓ Pass \\
\hline
\texttt{test\_circular\_buffer\_full} & Append 65 values to size-60 buffer & Buffer contains last 60 values only & ✓ Pass \\
\hline
\texttt{test\_format\_bytes} & Input: 1536 bytes & Output: ``1.5 KB'' & ✓ Pass \\
\hline
\texttt{test\_gpu\_unavailable} & Mock GPUtil import failure & Returns \texttt{available=False}, no exception & ✓ Pass \\
\hline
\texttt{test\_process\_sort\_cpu} & Set sort key to ``cpu'', collect processes & Processes sorted by CPU descending & ✓ Pass \\
\hline
\texttt{test\_memory\_history} & Collect 5 times & History contains 5 memory percentages & ✓ Pass \\
\hline
\end{tabular}
\end{table}
...
\hline
\texttt{test\_monitor\_widget\_data\_flow} & CPUMonitor → CPUWidget & Widget receives valid data dict & ✓ Pass \\
\hline
\texttt{test\_app\_all\_monitors\_init} & SystemMonitorApp startup & All 7 monitors instantiated & ✓ Pass \\
\hline
\texttt{test\_widget\_handles\_none\_data} & Widget render with \texttt{data=None} & Displays ``Loading...'' message & ✓ Pass \\
\hline
\texttt{test\_process\_kill\_confirmation} & User triggers kill without confirm & Process not terminated & ✓ Pass \\
\hline
\end{tabular}
\end{table}
...
\hline
CPU Usage (avg) & <1.0\% & 0.7\% & ✓ Met \\
CPU Usage (peak) & <2.0\% & 1.3\% & ✓ Met \\
Memory Usage & <50MB & 41MB & ✓ Met \\
Startup Time & <2s & 1.2s & ✓ Met \\
UI Update Latency & <100ms & 45ms & ✓ Met \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item Systop overhead increased minimally (0.7\% → 0.9\% CPU) under stress
...
Figure~\ref{fig:main-interface} shows systop running on an Arch Linux system with all monitoring widgets active.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{images/main_interface.png}
\caption{Main systop interface showing CPU, memory, disk, network, GPU, and process monitoring}
\label{fig:main-interface}
\end{figure}

\textbf{Interface Elements Visible}:
\begin{itemize}
    \item CPU widget: 4-core utilization bars, frequency info, 60-second history graph
    \item Memory widget: RAM/swap usage with visual bars and historical trend
    \item Disk widget: Partition usage, read/write speeds with ASCII graphs
    \item Network widget: Per-interface bandwidth with upload/download rates
    \item GPU widget: NVIDIA RTX 3060 metrics (utilization, temperature, memory)
    \item Process widget: Sortable process table with 100+ processes, search functionality
\end{itemize}

Figure~\ref{fig:process-management} demonstrates interactive process management with search active.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{images/process_management.png}
\caption{Process search and kill functionality with confirmation dialog}
\label{fig:process-management}
\end{figure}

Figure~\ref{fig:graceful-degradation} shows systop on a system without GPU, displaying ``N/A'' gracefully.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{images/no_gpu_fallback.png}
\caption{Graceful degradation when GPU unavailable—no crashes or errors}
\label{fig:graceful-degradation}
\end{figure}
...
\begin{itemize}
    \item ✓ Primary Objective 1: Unified monitoring interface implemented and operational
    \item ✓ Primary Objective 2: Real-time visualization with ASCII graphs functional
    \item ✓ Primary Objective 3: Interactive process management with sort/search/kill works
    \item ✓ Primary Objective 4: Modular architecture achieved (3 layers, 14 modules)
    \item ✓ Primary Objective 5: Robust error handling validated across 3 test systems
    \item ✓ Secondary Objective 6: Performance efficiency met (0.7\% CPU, 41MB RAM)
    \item ✓ Secondary Objective 7: Testing coverage achieved (74\% overall, 80\%+ monitors)
    \item ✓ Secondary Objective 8: Documentation comprehensive (README, docstrings, BUILD\_PLAN)
\end{itemize}
...
\textbf{Objective 1: Unified Monitoring Interface} — \textbf{ACHIEVED ✓}
...
\textbf{Objective 2: Real-Time Data Visualization} — \textbf{ACHIEVED ✓}
...
\textbf{Objective 3: Interactive Process Management} — \textbf{ACHIEVED ✓}
...
\textbf{Objective 4: Modular and Maintainable Architecture} — \textbf{ACHIEVED ✓}

\textit{Goal}: Design clean, modular architecture separating data collection, business logic, and presentation concerns.

\textit{Evidence}:
\begin{itemize}
    \item Three-layer architecture (Monitors → Widgets → App) with clear boundaries
...
\textbf{Objective 5: Robust Error Handling} — \textbf{ACHIEVED ✓}
...
\textbf{Objective 6: Performance Efficiency} — \textbf{ACHIEVED ✓}
...
\textbf{Objective 7: Comprehensive Testing} — \textbf{ACHIEVED ✓}
...
\textbf{Objective 8: User-Friendly Documentation} — \textbf{ACHIEVED ✓}
...
\textbf{Objective 9: Cross-Platform Foundation} — \textbf{PARTIALLY ACHIEVED ⚠}
...
\textbf{Objective 10: Professional Development Practices} — \textbf{ACHIEVED ✓}

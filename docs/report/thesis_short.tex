...
Performance evaluation shows that systop achieves its objectives with minimal resource overhead (0.2\% CPU, ~66MB memory), providing users with an efficient, feature-rich alternative to existing monitoring tools. The complete source code, documentation, and test suite serve as an educational resource for software development students and contribute to the open-source Linux ecosystem.
...
\begin{itemize}
    ...
    \item \textbf{systop}: Target overhead (<1\% CPU, ~70MB memory)
\end{itemize}
...
\textbf{Update Tuning}: CPU 1s, disk 5s, reducing calls. \textbf{Efficient Structures}: \texttt{deque} with fixed \texttt{maxlen} provides O(1) operations. \textbf{Lazy Evaluation}: Graphs computed only when rendered. \textbf{Async Updates}: Textual reactive properties prevent blocking. \textbf{Memory}: Bounded buffers ~66MB. \textbf{CPU}: 0.2\%, exceeding <1\% requirement. \textbf{Graceful Degradation}: Single startup check, no retries.
...
\begin{itemize}
    ...
    \item Memory: 66MB total (48MB baseline + 10MB monitors + 8MB widgets)
\end{itemize}
...
\textbf{Unit Testing}: 115 tests with mocking for system calls, covering utilities (23 tests), formatters (52 tests), and individual monitors (40 tests). \textbf{Integration Testing}: 23 tests for component interactions and data flow validation. \textbf{System Testing}: 4 end-to-end tests including application startup and multi-component coordination. \textbf{Acceptance}: Manual verification across three systems with comprehensive feature checklist.

\textbf{Test Execution Results}: All 142 tests passed in 11.11 seconds with 0\% failure rate, demonstrating robust implementation and comprehensive test coverage across all modules.
...
\small
\begin{tabular}{|p{3.5cm}|p{4cm}|p{3cm}|p{1.5cm}|}
\hline
\textbf{Test Case} & \textbf{Input/Setup} & \textbf{Expected Output} & \textbf{Result} \\
\hline
\raggedright\small\texttt{test\_cpu\_\allowbreak{}collect} & Call \texttt{CPUMonitor.\allowbreak{}collect()} & Dict with CPU metrics, valid ranges & Pass \\
\hline
\raggedright\small\texttt{test\_circular\_\allowbreak{}buffer\_full} & Append 65 values to size-60 buffer & Buffer contains last 60 values only & Pass \\
\hline
\raggedright\small\texttt{test\_format\_\allowbreak{}bytes} & Input: 1536 bytes & Output: ``1.5 KB'' & Pass \\
\hline
\raggedright\small\texttt{test\_gpu\_\allowbreak{}unavailable} & Mock GPUtil import failure & Returns \texttt{available=\allowbreak{}False}, no exception & Pass \\
\hline
\raggedright\small\texttt{test\_process\_\allowbreak{}sort\_cpu} & Set sort key to ``cpu'', collect processes & Processes sorted by CPU descending & Pass \\
\hline
\raggedright\small\texttt{test\_memory\_\allowbreak{}history} & Collect 5 times & History contains 5 memory percentages & Pass \\
\hline
\end{tabular}
\end{table}
...
\hline
\raggedright\small\texttt{test\_monitor\_\allowbreak{}widget\_data\_\allowbreak{}flow} & CPUMonitor $\rightarrow$ CPUWidget & Widget receives valid data dict & Pass \\
\hline
\raggedright\small\texttt{test\_app\_all\_\allowbreak{}monitors\_init} & SystemMonitorApp startup & All 7 monitors instantiated & Pass \\
\hline
\raggedright\small\texttt{test\_widget\_\allowbreak{}handles\_none\_\allowbreak{}data} & Widget render with \texttt{data=None} & Displays ``Loading...'' message & Pass \\
\hline
\raggedright\small\texttt{test\_process\_\allowbreak{}kill\_confirm} & User triggers kill without confirm & Process not terminated & Pass \\
\hline
\end{tabular}
\end{table}
...
\subsection{Test Execution Breakdown}

\begin{table}[h]
\centering
\caption{Test execution results by module}
\label{tab:test-execution}
\small
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Test Module} & \textbf{Tests} & \textbf{Result} & \textbf{\% of Total} \\
\hline
\texttt{test\_formatters.py} & 52 & Pass & 36.6\% \\
\texttt{test\_history.py} & 23 & Pass & 16.2\% \\
\texttt{test\_processes.py} & 16 & Pass & 11.3\% \\
\texttt{test\_gpu.py} & 10 & Pass & 7.0\% \\
\texttt{test\_network.py} & 9 & Pass & 6.3\% \\
\texttt{test\_sensors.py} & 9 & Pass & 6.3\% \\
\texttt{test\_memory.py} & 7 & Pass & 4.9\% \\
\texttt{test\_cpu.py} & 6 & Pass & 4.2\% \\
\texttt{test\_disk.py} & 6 & Pass & 4.2\% \\
\texttt{test\_integration.py} & 4 & Pass & 2.8\% \\
\hline
\textbf{Total} & \textbf{142} & \textbf{100\% Pass} & \textbf{100\%} \\
\hline
\multicolumn{4}{|l|}{\textbf{Execution Time:} 11.11 seconds} \\
\hline
\end{tabular}
\end{table}

\textbf{Test Distribution Analysis}: The test distribution reflects the complexity of each module. Formatters (36.6\%) and history utilities (16.2\%) account for over half of all tests due to comprehensive edge case coverage. Process monitoring (11.3\%) requires extensive testing for sorting, filtering, and signal handling. The 11.11-second execution time demonstrates efficient test design, averaging 78ms per test.

\section{Performance Evaluation}
\label{sec:performance-evaluation}

\textbf{Test Environment}: Arch Linux, AMD Ryzen 7 4800H (8C/16T), 16GB RAM.
...
\hline
CPU Usage (avg) & <1.0\% & 0.2\% & $\checkmark$ Met \\
CPU Usage (peak) & <2.0\% & 0.5\% & $\checkmark$ Met \\
Memory Usage & <70MB & 66MB & $\checkmark$ Met \\
Startup Time & <5s & 4.95s & $\checkmark$ Met \\
UI Update Latency & <100ms & 45ms & $\checkmark$ Met \\
\hline
\end{tabular}
\end{table}

\textbf{Startup Time Breakdown}: Total execution time of 4.95 seconds comprises user CPU time (1.02s) for Python interpretation and data collection initialization, system CPU time (1.85s) for system calls and I/O operations, and initialization overhead (2.08s) for library imports and UI rendering. The Python runtime accounts for the majority of startup latency compared to compiled alternatives, though this is a one-time cost for long-running monitoring sessions.
...
\begin{itemize}
    ...
    \item $\checkmark$ Performance efficiency (0.2\% CPU, 66MB RAM)
...
\begin{itemize}
    \item \textbf{Performance Characteristics}: Systop consumes 0.2\% CPU and 66MB memory, representing negligible impact (<1\% on 8GB systems). At 10× below the 2-3\% monitoring overhead threshold, systop enables 24/7 operation without degrading system performance, suitable even for resource-constrained environments. The 1-second update interval balances responsiveness and overhead.
    
    \item \textbf{Testing Coverage}: 74\% overall coverage (83-89\% core modules) with 142 tests demonstrates reliability. Lower UI coverage (63\%) acceptable as manual testing required. 100\% pass rate validates quality.
    
    \item \textbf{Feature Completeness}: All 10 objectives achieved. Unified interface improves efficiency, historical graphs enable pattern recognition, graceful degradation adapts to diverse hardware.
    
    \item \textbf{Comparative Performance}: Systop's 66MB memory (between btop's 28MB and glances' 45MB) and 0.2\% CPU usage are production-ready. Python adds memory overhead but achieves excellent CPU efficiency.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Clean Architecture}: Three-layer design with BaseMonitor abstraction follows SOLID principles.
    
    \item \textbf{Robust Error Handling}: Graceful fallbacks for missing hardware ensure stability across diverse systems.
    
    \item \textbf{Efficient Data Structures}: Circular buffer provides O(1) operations with fixed memory footprint.
    
    \item \textbf{Performance}: Async I/O and efficient structures achieve <1\% CPU usage.
    
    \item \textbf{Systematic Methodology}: 16-step BUILD\_PLAN.md achieved on-schedule delivery.
    
    \item \textbf{Comprehensive Testing}: 142 tests ensure reliability with 100\% pass rate.
    
    \item \textbf{Extensive Documentation}: Complete multi-level documentation for users and developers.
    
    \item \textbf{Learning Resource}: Demonstrates engineering principles with readable code.
    
    \item \textbf{Practical Utility}: Production-ready tool with simple pip installation.
    
    \item \textbf{Modern Technology}: Showcases Python 3.13, Textual framework, and contemporary practices.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Limited History Buffer}: Fixed 60-second buffer without configurability or persistence.
    
    \item \textbf{No Configuration Persistence}: Settings reset on each launch; no config file support.
    
    \item \textbf{Single-System Monitoring}: No remote or distributed system monitoring capabilities.
    
    \item \textbf{NVIDIA GPU Only}: Limited to NVIDIA GPUs; no AMD or Intel GPU support.
    
    \item \textbf{No Alert System}: Lacks threshold-based notifications or warnings.
    
    \item \textbf{Linux-Only}: Not cross-platform; requires Linux-specific interfaces.
    
    \item \textbf{Terminal Size Requirements}: Minimum 80×24 terminal with Unicode support.
    
    \item \textbf{Python Performance Ceiling}: Interpreted language limits maximum efficiency.
    
    \item \textbf{Basic Process Management}: Limited to sorting, filtering, and kill operations.
    
    \item \textbf{Test Coverage Gaps}: 26\% code uncovered, particularly in UI components.
    
    \item \textbf{Limited Platform Testing}: Tested only on Arch/Ubuntu/Debian distributions.
    
    \item \textbf{No Accessibility Features}: Lacks screen reader support and alternative color schemes.
\end{itemize}

\section{Recommendations for Future Development}
\label{sec:recommendations}

\textbf{High-Priority Enhancements:}
\begin{itemize}
    \item \textbf{Configuration File Support} (10-15h): TOML/YAML config for persistent settings.
    
    \item \textbf{AMD/Intel GPU Support} (20-25h): Extend GPU monitoring beyond NVIDIA.
    
    \item \textbf{Data Export} (8-12h): CSV/JSON export for external analysis.
    
    \item \textbf{Enhanced Process Details} (25-30h): Open files, network connections, thread info.
\end{itemize}

\textbf{Medium-Priority Enhancements:}
\begin{itemize}
    \item \textbf{Alert System} (15-20h): Threshold-based notifications for critical metrics.
    
    \item \textbf{Extended Retention} (20-30h): Configurable history beyond 60 seconds.
    
    \item \textbf{Customizable Layout} (30-40h): User-defined widget arrangement and sizing.
    
    \item \textbf{Remote Monitoring} (60-80h): Monitor remote systems over SSH/network.
\end{itemize}

\textbf{Lower-Priority Enhancements:}
\begin{itemize}
    \item \textbf{Cross-Platform Support} (40-60h): Abstract platform-specific code for macOS and Windows.
    
    \item \textbf{Web Dashboard} (80-100h): Optional web interface with browser-based monitoring.
    
    \item \textbf{Plugin System} (60-80h): Plugin API for custom monitors and third-party extensions.
\end{itemize}

\textbf{Quality and Maintenance:}
\begin{itemize}
    \item \textbf{Testing}: Increase coverage to 80\%+, add UI tests, implement CI/CD, performance regression tests.
    
    \item \textbf{Documentation}: Video tutorials, contribution guidelines, troubleshooting section, internationalization.
    
    \item \textbf{Accessibility}: Screen reader compatibility, color blindness schemes, high-contrast mode.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Layered Architecture}: Separating monitors, widgets, and application layers proved successful. Each layer developed independently, delivering anticipated testability and maintainability benefits. Would repeat in future projects.
    
    \item \textbf{BaseMonitor Abstract Class}: Provided consistent interface and shared functionality. While inheritance worked well, protocol-based approach might be more Pythonic in future.
    
    \item \textbf{Circular Buffer}: Custom implementation provided clean API and flexibility worth the modest overhead.
\end{itemize}

\textbf{Technology Choices:}
\begin{itemize}
    \item \textbf{Python}: Development speed and library ecosystem justified ~15-20MB overhead. Completed in 210 hours vs estimated 300-400 in C++. Appropriate for academic context.
    
    \item \textbf{Textual Framework}: Enabled rapid UI development with reactive updates. Learning curve and occasional bugs outweighed by benefits.
    
    \item \textbf{psutil}: Saved hundreds of hours implementing /proc parsing. Well-tested abstractions handled edge cases automatically.
\end{itemize}

\textbf{Implementation Decisions:}
\begin{itemize}
    \item \textbf{Graceful Degradation}: Error handling for optional hardware ensured universal compatibility. Testing effort justified by robustness.
    
    \item \textbf{Fixed 60-Second History}: Simple approach adequate. Would add configurability in next iteration while keeping sensible defaults.
    
    \item \textbf{Continuous Testing}: Writing tests alongside implementation caught bugs early with 100\% pass rate. Superior to deferred testing.
\end{itemize}

\textbf{Key Lessons Learned:}
\begin{itemize}
    \item \textbf{Start Simple}: Incremental feature addition prevented overwhelming complexity.
    
    \item \textbf{Defensive Programming}: Comprehensive error handling prevented debugging hours and user frustration.
    
    \item \textbf{Testing Enables Confidence}: 142 automated tests enabled fearless refactoring.
    
    \item \textbf{Documentation Contemporaneously}: Writing documentation during development easier than deferring to end.
    
    \item \textbf{Performance Early}: Continuous profiling prevented entrenched performance problems.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Textual Framework (20-25 hours)}: Started from zero knowledge to proficient implementation. Learning approach included documentation study, example analysis, incremental building, source code examination, and community engagement. Key challenges included understanding the reactive system, CSS-like styling, event propagation, and debugging asynchronous components.
    
    \item \textbf{psutil and System Libraries (~15 hours)}: Mastered through documentation reading, REPL exploration, comparative analysis with other tools, and edge case testing. Required understanding OS concepts beyond just API usage, including /proc filesystem structure and kernel interfaces.
    
    \item \textbf{pytest and Development Tools (~30 hours)}: Continuous learning throughout project on fixtures, mocking, parametrization, and coverage analysis. Key insight: Testing alongside implementation proved easier than deferred testing. Also enhanced skills in Git workflows, virtual environments, and development tools.
    
    \item \textbf{Effective Learning Strategies}: Documentation-first approach, building mental models before coding, hands-on experimentation, incremental complexity increase, consulting multiple sources, time-boxed exploration to avoid rabbit holes, prioritizing contemporary documentation, and embracing initial discomfort with unfamiliar concepts.
\end{itemize}
...
All targets exceeded: 0.2\% CPU (80\% margin), 66MB memory (6\% margin), <100ms latency, 200+ process scalability, no memory leaks. Competitive with btop/glances while providing Python accessibility.
...
0.2\% CPU (80\% below target), 66MB memory (6\% below target), validated across multiple runs.
